...
def extract_external_links(html_content,link_list, mainurl):
	...
	header = parser.title.string
	    ...
	    sumary = allstr
	if BeautifulSoup(urllib.urlopen("http://data.alexa.com/data?cli=10&dat=s&url="+ mainurl), "xml"):
                alexa_score = BeautifulSoup(urllib.urlopen("http://data.alexa.com/data?cli=10&dat=s&url="+ mainurl), "xml").find("REACH")['RANK']
        else:
                alexa_score = 1.9

        dict1= {"scores":[],"link":mainurl ,"data": allstr,"header":header,"votes":alexa_score, "entity": entity_list, "flagindex":indexvar, "sum": sumary, "img": image}
