...
def parse(url):
	...
	for grams in bigrams2:
		...
		if T.get(grams[0]+' '+grams[1]+' '+grams[2]+' '+grams[3],"not_exsist")!="not_exsist":
			...
			Techentitity_list.append(T.get(grams[0]+' '+grams[1]+' '+grams[2]+' '+grams[3],"not_exsist")[0])
	...
	if techscore<2.9 and bizscore<2.9 and energyscore<2.9:
		#print "Other  Category "+str(techscore)+' '+ str(bizscore) + ' '+str(energyscore)  
		flagindex ="Other Category"	
	else:
		techscore= Techscore/tech_count
		energyscore=  Energyscore/enrrgy_count
		bizscore= BusinensScore/biz_count
		print techscore, tech_count
		#print enrgyscore, enrrgy_count
		print 	bizscore, biz_count
	...
	if len(TechList)>10:
		TechList=TechList[0:10]
	#print TechList
	#score_list=[-0.2,2 ,0.12,0.92]
	#print alexa_score
	return text1, header, score_list, 1/(1+math.log10(int(alexa_score))), flagindex, TechList 
